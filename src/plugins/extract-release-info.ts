...
import { VersionOptionSpec, isSpecialVersionOption, parseVersionOption } from '../options.js';
...
const attacher: Plugin<[string], Root, Root> = function extractUnreleasedContents(version: string) {
  // eslint-disable-next-line @typescript-eslint/unbound-method
  const processorData = this.data;
  const target = parseVersionOption(version);

  if (!target) {
    throw new BoneheadedError('Invalid version option, should already be validated');
  }
